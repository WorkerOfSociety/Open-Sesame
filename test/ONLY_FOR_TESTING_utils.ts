...
export async function get_country_url(country: string): Promise<string> {
    const geocode_url = `https://api.geoapify.com/v1/geocode/search?text=${encodeURIComponent(country)}&type=country&apiKey=${process.env.GEOAPIFY_KEY}`;
    ...
    return `https://maps.geoapify.com/v1/staticmap?format=png&style=toner&width=600&height=400&center=lonlat:${lon},${lat}&zoom=10&scaleFactor=2&apiKey=${process.env.GEOAPIFY_KEY}`;
}
